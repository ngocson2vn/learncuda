...
__device__ void gen_indices(int* d_mma_indices, const int* d_mma_shape, const int* d_mma_stride, int tid) {
  ...
  // Each warp is responsible for 16 rows
...
__device__ void tma_mmav5_fn(
  ...
  *tmem_handle_ptr = 0;
  ...
  if (warp_id < 8) {
    ...
    // 6. Arrive at 1st CTA sync point
    sync_cta_unaligned();


    // 7. Arrive at 2nd CTA sync point
    sync_cta_unaligned();
    ...
    ((volatile bool*)smem.done)[tid] = true;
    bool done = ((volatile bool*)smem.done)[tid];
    printf("[tid = %u] smem.done[%u] = %u\n\n", tid, tid, done);
  #endif
  ...
  //====================================================================================
    ...
    // 3. MMA the first 64x64xf16 tile_m and tile_n
    ...
    // 32 threads in warp 9 is also synced at this point
    uint32_t is_leader = elect_leader_sync();
    ...
    #pragma unroll(1)
    while (k_tile_counter > 0) {
      // Sync warp 9 to ensure that stage_counter and parity_phase are identical
      // accross 32 threads
      sync_warp();
      ...
      // The final (tile_m, tile_n) was consumed
  ...
  // Warp 0-11 arrive at the 3rd CTA sync point
  sync_cta_unaligned();
}
/*===---------------------------------------------------------------------------------------------------------------===*/
...
void matmul_cpu_tile(ABType* mat_A, ABType* mat_B, DType* mat_D, int a_stride[2], int b_stride[2], int d_stride[2], int M, int N, int K, int m_start, int n_start) {
  ...
  for (int i = 0; i < M_TILE; i++) {
    ...
    for (int j = 0; j < N_TILE; j++) {
      ...
      for (int k = 0; k < K; k++) {
        auto mat_A_mk = static_cast<DType>(mat_A[m * a_stride[0] + k * a_stride[1]]);
        auto mat_B_nk = static_cast<DType>(mat_B[n * b_stride[0] + k * b_stride[1]]);
...
void matmul_cpu_parallel(ABType* mat_A, ABType* mat_B, DType* mat_D, int a_stride[2], int b_stride[2], int d_stride[2], int M, int N, int K) {
...
__global__ void blackwell_mmav5_f16_fp32(
  ...
  tma_mmav5_fn<ABType, float, M_TILE, N_TILE, K_TILE, NUM_STAGES>(a_tensor_map, b_tensor_map, d_tensor_map, M, N, K);
}
} // End extern "C"

[[maybe_unused]] static const char* kernel_name = "blackwell_mmav5_f16_fp32";
...
[[maybe_unused]] static const char* kernel_name = "blackwell_mmav5_bf16_fp32";
...
int main(int argc, char** argv) {
...
#if defined(DEBUG_MODE)
  printf("Mode: DEBUG_MODE\n");
#elif defined(PROFILE_MODE)
  printf("Mode: PROFILE_MODE\n");
#else
  printf("Mode: VERIFY_MODE\n");
#endif
  ...
  // Threadblock: 320 threads (10 warps)
  //   - Warp 0-7: Allocate TMEM and move matrix D from TMEM -> SMEM
  //   - Warp 8: Issues TMA ops to copy matrices A and B from GMEM -> SMEM
  //   - Warp 9: Issues MMA ops
  constexpr int THREADS_PER_BLOCK = 320;
...
/*===-----------------------------------------------------------------------------------------------===*/
  ...
  // Check FLOPS
  printf("\nStart profiling kernel with max_round_count = %d\n", max_round_count);
  timer.start();
  ...
  printf("Finish profiling kernel\n");

  double gemm_time = total_seconds / max_round_count;
...
/*===-----------------------------------------------------------------------------------------------===*/
  ...
  bool dump_data = (M * N < 1024 * 1024);
  if (dump_data) {
    file_ptr = get_file_ptr("output.txt");
    const char a_indices[] = {'m', 'k'};
    fprint_mat(file_ptr, "h_A", h_A, a_indices, a_shape, a_stride);
    fprintf(file_ptr, "\n\n");

    const char b_indices[] = {'n', 'k'};
    fprint_mat(file_ptr, "h_B", h_B, b_indices, b_shape, b_stride);
    fprintf(file_ptr, "\n\n");
  }
  ...
  matmul_cpu_parallel<ABType, DType, M_TILE, N_TILE, MAX_THREAD_COUNT>(
    h_A.data(), h_B.data(), h_D_cpu.data(),
    a_stride, b_stride, d_stride, M, N, K
  );

  if (dump_data) {
    printf("Dumping cpu and gpu D matrices\n");
    const char d_indices[] = {'m', 'n'};
    fprint_mat(file_ptr, "h_D_cpu", h_D_cpu, d_indices, d_shape, d_stride);
    fprintf(file_ptr, "\n\n");
    fprint_mat(file_ptr, "h_D_gpu", h_D_gpu, d_indices, d_shape, d_stride);
  }
  ...
  for (int i = 0; i < MAX_THREAD_COUNT; i++) {
    ...
    verifyThreads.emplace_back(
      verify<DType>, 
      h_D_cpu.data(), h_D_gpu.data(), idx_start, kNumElements
    );
  }
